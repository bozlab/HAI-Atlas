...
async function main() {
  ...
  fs.createReadStream("prisma/HAI-Atlas-Paper-final_links.csv") // Adjust file path as per your setup
    ...
    .on("data", async (row) => {
      try {
        ...
        if (!mainLink || mainLink === "") {
          console.error(
            "Main Link is missing or empty for the document:",
            row.name
          );
        ...
        for (const countryName of countryNames) {
          ...
          if (country) {
            ...
            console.error(
              `Country "${countryName}" not found in the database. Skipping record.`
            );
        ...
        nlpLines.forEach((line: any) => {
          ...
          const mappedKey =
            nlpKeyMapping[key] || key.toLowerCase().replace(/\s+/g, "");
        ...
        const resource = await prisma.resource.create({
          data: {
            ...
            organizationType: row.organizationType
              .split(",")
              .map((type: string) => type.trim()), // Split by commas
            publisher: row.publisher
              .split(",")
              .map((pub: string) => pub.trim()), // Handle multiple publishers
    ...
    .on("end", () => {
      console.log(
        `Finished processing CSV. Total resources added: ${addedResourcesCount}`
      );
    });
}
